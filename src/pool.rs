...
impl DataArg {
    pub fn new(addr: String, path: String, mut headers: Vec<(String, String)>, 
               data: &[u8]) -> DataArg{
...
pub fn put_job(data_arg: DataArg) {
    ...
    pool.execute(move || {
        ...
        let (addr, path, data, headers) = (&data_arg.addr, &data_arg.path, &data_arg.data, &data_arg.headers);
...
mod tests {
    ...
    #[test]
    fn test_pool() {
        let timestamp = ("x-timestamp".to_owned(), format!("{}", time::get_time().sec));
        let xt = ("x-t".to_owned(), format!("{}", "9bdbb499e4c9deed2c4a3e355ea2d580"));
        let headers = vec![timestamp, xt];
        let arg = DataArg::new("127.0.0.1:17172".to_owned(), 
